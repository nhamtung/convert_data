...
boost::thread* distance_object_thread;
sensor_msgs::Image distance_object_image;
...
double WIDTH_IMAGE_RESIZE, HEIGHT_IMAGE_RESIZE, DISTANCE_OBJECT;
...
std::string dist_str;
uint8_t offset_pixel = 2;

void mouseDistance(sensor_msgs::Image resize_image, uint32_t offset, double x_mouse_, double y_mouse_);
void distanceObjectThread(ros::Rate loop_rate);
...
void depthImageCallback(const sensor_msgs::Image::ConstPtr& msg){
    ...
    distance_object_image = resize_image;
    mouseDistance(resize_image, offset_pixel, x_mouse, y_mouse);
    // distanceObject(resize_image);
...
void mouseDistance(sensor_msgs::Image resize_image, uint32_t offset, double x_mouse_, double y_mouse_){
    ...
    if(checkSize(resize_image, offset, offset)){
        dist_image = resizeImage(resize_image, offset, offset, x_mouse_, y_mouse_);
    }
...
void distanceObjectThread(ros::Rate loop_rate){
    while (ros::ok()){
        ...
        double dist;
        uint8_t num_dist = 0;
        uint8_t offset = offset_pixel;
        if(checkSize(distance_object_image, offset, offset)){
            for(uint16_t i=offset; i<HEIGHT_IMAGE_RESIZE; i=i+offset){
                for(uint16_t j=0; j<WIDTH_IMAGE_RESIZE; j=j+offset){
                    dist_image = resizeImage(distance_object_image, offset, offset, i, j);
                    dist = getDistance(dist_image);
                    if (dist > 0.3 && dist < DISTANCE_OBJECT){
                        num_dist++;
                        ROS_INFO("detect_object.cpp - num_dist: %d - dist: %f -  pixel: (%d, %d)", num_dist, dist, i, j);
                        break;
                    }
                }
                if(num_dist > 5){
        ...
        if(num_dist>1){
            ROS_WARN("detect_object.cpp - OBJECT");
            num_dist = 0;
        }
        ros::spinOnce();
        loop_rate.sleep();
...
void distanceObject(sensor_msgs::Image resize_image){
    sensor_msgs::Image dist_image;
    double dist;
    uint8_t num_dist = 0;
    uint8_t offset = 4;
    if(checkSize(resize_image, offset, offset)){
        for(int i=0; i<HEIGHT_IMAGE_RESIZE; i=i+offset){
            for(int j=0; j<WIDTH_IMAGE_RESIZE; j=j+offset){
                dist_image = resizeImage(resize_image, offset, offset, i, j);
                dist = getDistance(dist_image);
                if (dist < DISTANCE_OBJECT){
                    num_dist++;
                    dist_str = std::to_string(dist) + " m";
                }
            }
    ...
    if(num_dist>3){
        ROS_INFO("detect_object.cpp - OBJECT");
    }
}
...
void loadParameters(ros::NodeHandle n, std::string node_name){
  ...
  n.param<double>(node_name + "/distance_object", DISTANCE_OBJECT, 1.0);
  ROS_INFO("distance_object: %0.0f", DISTANCE_OBJECT);
}
...
int main(int argc, char **argv)
{
    ...
    distance_object_thread = new boost::thread (&distanceObjectThread, loop_rate);
